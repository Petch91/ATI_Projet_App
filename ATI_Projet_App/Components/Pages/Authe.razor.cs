        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            ...
            if (firstRender)
            {
                ...
                try
                {
                    ...
                    using (HttpResponseMessage response = _client.GetAsync($"http://192.168.122.53:7000/api/user/token/{Id}/{HashToken}").Result)
                    {
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            if (token != null)
                            {
                                ...
                                if (jwt.ValidTo >= DateTime.UtcNow)
                                {
                                    ...
                                    using (HttpResponseMessage res = _client.GetAsync("http://192.168.122.53:7000/api/user/byid/" + id).Result)
                                    {
