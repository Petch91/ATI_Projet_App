if (!window.BlazorBootstrapPerso) {
    window.BlazorBootstrapPerso = {};
}
...
window.BlazorBootstrapPerso = {
    ...
    autocomplete: {
        ...
        focusListItem: (ul, key, startIndex) => {
            ...
            if (key === window.BlazorBootstrapPerso.fe.ARROWDOWN) {
                if (nextSelectedIndex < childNodes.length - 1)
                    nextSelectedIndex++;
            }
            else if (key === window.BlazorBootstrapPerso.fe.ARROWUP) {
                if (nextSelectedIndex > 0 && nextSelectedIndex <= childNodes.length - 1)
                    nextSelectedIndex--;
            }
            else if (key === window.BlazorBootstrapPerso.fe.HOME) {
                nextSelectedIndex = 0;
            }
            else if (key === window.BlazorBootstrapPerso.fe.END) {
            ...
            currencyEl?.addEventListener('beforeinput', function (event) {
                if (event.inputType === 'insertFromPaste' || event.inputType === 'insertFromDrop') {
                    ...
                    if (event.data && event.data.length > 0) {
                        if (BlazorBootstrapPerso.hasInvalidChars(event.data, validChars))
                            event.preventDefault();
                    }
                }
            });
    ...
    googlemaps: {
        addMarker: (elementId, marker, dotNetHelper) => {
            let mapInstance = window.BlazorBootstrapPerso.googlemaps.get(elementId);
            if (mapInstance) {
                ...
                window.BlazorBootstrapPerso.googlemaps.markerEls[elementId].push(markerEl);
        ...
        create: (elementId, map, zoom, center, markers, clickable) => {
            window.BlazorBootstrapPerso.googlemaps.instances[elementId] = {
                map: map,
                zoom: zoom,
                center: center,
                markers: markers,
        ...
        get: (elementId) => {
            return window.BlazorBootstrapPerso.googlemaps.instances[elementId];
        },
        initialize: (elementId, zoom, center, markers, clickable, dotNetHelper) => {
            window.BlazorBootstrapPerso.googlemaps.markerEls[elementId] = window.BlazorBootstrapPerso.googlemaps.markerEls[elementId] ?? [];
            ...
            window.BlazorBootstrapPerso.googlemaps.create(elementId, map, zoom, center, markers, clickable);

            if (markers) {
                for (const marker of markers) {
                    window.BlazorBootstrapPerso.googlemaps.addMarker(elementId, marker, dotNetHelper);
                }
            }
        },
        instances: {},
        ...
        updateMarkers: (elementId, markers, dotNetHelper) => {
            let markerEls = window.BlazorBootstrapPerso.googlemaps.markerEls[elementId] ?? [];
